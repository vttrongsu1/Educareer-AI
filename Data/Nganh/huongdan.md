...
* **Quy định nghiêm ngặt về liên kết (Links):** Tất cả các liên kết trong trường `link` phải là liên kết thực tế, đang hoạt động tốt (live URL) dẫn đến bài báo chính thống, trang tuyển sinh chính thức hoặc trang chủ của trường đại học. **Tuyệt đối không sử dụng** các liên kết giả lập, placeholder như `[Link thực tế]`, `http://example.com`, `http://google.com`, hoặc điền chung chung `(Không có thông tin)`.
...
```json
{
  ...
  "chung": {
    ...
    "nguon_tham_khao": [
      { "chu_de": "Điểm chuẩn & tổ hợp xét tuyển năm mới nhất", "link": "[Link thực tế hoặc (Không có thông tin)]" },
      { "chu_de": "Thông tin tuyển sinh", "link": "[Link thực tế hoặc (Không có thông tin)]" },
      { "chu_de": "Mức lương ngành", "link": "[Link thực tế hoặc (Không có thông tin)]" }
    ]
  },
